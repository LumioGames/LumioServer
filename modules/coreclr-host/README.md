...
`coreclr-host` 是 Rust Host 与 Managed 世界之间的唯一桥梁。一个进程只启动一个 CoreCLR、只加载一个 CoreEngine 包、只服务一个 GameRelease（决策门 D-001 默认值）；Gameplay 以 Collectible ALC 装载并可热重载。本模块保证跨边界只有稳定 ABI 与稳定错误码，Managed 异常与 Rust panic 都不会以原始形态穿过边界，可恢复的 Gameplay 异常与进程级崩溃**绝不**混为一类。
...
- 故障分级裁决：可捕获 Gameplay Exception → 隔离为 Session Fault（交 [session](../session/README.md) 处置）；Stack Overflow、OOM、CoreCLR 崩溃、Native UB → 进程级故障（交 [process](../process/README.md) 处置）。
...
- **输入**：ReleaseManifest 中的 Assembly Hash 与 ABI 要求（经 [release-router](../release-router/README.md) 校验后转入）、热重载请求（经维护编排）、Tick 调用（来自 Simulation Owner Thread）。
- **输出**：Runtime Tick 入口封装、稳定 Error Code、故障分级裁决、ALC 状态迁移事件。
- **稳定接口**：`load_runtime(manifest) -> RuntimeHandle | StableError`；`load_gameplay(alcRequest) -> ScopeHandle | StableError`；`tick_entry(slotContext, batch) -> TickResult | FaultClass`；`unload_gameplay(scope) -> Ok | LeakEvidence`。

## 上游与下游依赖

- **上游**：[world-slot](../world-slot/README.md)（Tick 入口调用、World 创建前的 ABI 校验）、[process](../process/README.md)（启动/关闭编排）、[maintenance](../maintenance/README.md)（经编排的热重载/停机）。
...
- 热重载：新模块装入新 ALC → 校验 Hash/签名/ABI → Quiesce 旧 Scope → 切换 → 卸载旧 ALC；失败时卸载新模块并保留最后有效活动模块（架构源 ADR-014 失败语义）。
...
- **失败路径**：
  ...
  - Gameplay Exception（可捕获）：转稳定 Error Code，裁决为 Session Fault，隔离该 Session，Slot 与其他 Session 不受影响。
...
- **测试面**：CoreCLR Smoke（启动/装载/卸载）、ABI 兼容与指针宽度失败、panic/Exception 双向转换、stale-handle、重复装载拒绝、取消/超时/销毁后完成、ALC 卸载与 Root 验证（架构源 ADR-006 验证清单）。
- **故障矩阵**：Gameplay 异常隔离为 Session Fault、ALC 泄漏、热重载失败回退、CoreCLR 崩溃后的进程恢复演练。
- **性能指标**：FFI Batch 大小与单次跨边界开销、热重载停顿时长（Quiesce 到切换完成）、GC 暂停对 Tick 尾延迟的贡献。

## 对应 ADR、Schema 与 Fixture

- 架构源 `docs/adr/ADR-006-native-managed-abi.md`（ABI、Loader、故障域）、`docs/adr/ADR-014-platform-capability.md`（热重载失败语义、CoreCLR 默认）。
...
- **D-001**（一进程一 Release）：临时默认值即本设计（每进程一个 `gameReleaseId`）；启用进程内多 Release 装载需新 ADR 并更新架构源 ADR-006/012 与 Loader Schema。登记见 [modules/README.md](../README.md) §11.1。
