...
`persistence-host` 拥有"恢复所需的一切字节如何安全落盘、如何被找回"的宿主侧编排，是 **WAL/TxnJournal/CommandLog 持久队列的唯一所有者**；Audit 队列归 [observability](../observability/README.md)，两个所有者可共享底层 IO 原语但不共享队列状态或 ack 通道（架构源 ADR-011，v1.1）。Canonical 字节格式与 `Encode/Decode` 契约归 Runtime（架构源 ADR-010），域 payload Schema 归 Voxel/Game；本模块负责的是 durability：staging、校验、fsync、原子替换、保留策略、恢复定位与只重放已提交记录。写入返回**显式 commit ack**——它只证明恢复输入已持久，不蕴含 Audit durable ack（双 ack 独立，编排方分别等待）。失败的写入永远不能污染最近有效 Checkpoint。
...
- Checkpoint 调度：周期与保留数量（SRV-D-009）。调度条件"每 N 单调秒或每 M Tick 取先到者"的两个分量各居其时钟域：单调间隔经 [host-runtime](../host-runtime/README.md) Timer 投递提醒命令，Tick 计数在 Simulation Owner Thread 的 Tick Barrier 上评估——**触发动作恒在 Tick Barrier 执行**，Timer 只是提醒，不产生第二个快照发起点。保留最近有效 Checkpoint 与失败 Bundle，升级不覆盖旧 Release/Snapshot。
...
- 磁盘压力处置：磁盘满/IO 错误时 durable 队列拒绝新命令，并以 `DiskPressure` 事件上报 [world-slot](../world-slot/README.md) 聚合根裁决（关闸或进维护）；本模块只报事实，不发起维护。
...
- 不决定何时做 SnapshotCut（Tick Barrier 语义归 Runtime；维护触发归 [maintenance-agent](../maintenance-agent/README.md)）。
...
- **输入**：Runtime 产出的 Canonical Snapshot 字节与 WAL/Journal 记录（经 Simulation Owner Thread 提交到持久队列）、恢复请求（来自 [process](../process/README.md) 启动流程）、维护落盘请求（来自 [maintenance-agent](../maintenance-agent/README.md) 编排）。
- **输出**：commit ack（含 `snapshotId` 与 Hash，供 world-slot 与 maintenance-agent 分别消费）、恢复输入流（交 Runtime materialize）、磁盘压力状态事件、Snapshot 元数据（本仓只保存元数据与句柄，不解析内部状态）。
- **稳定接口**：`persist_snapshot(cut) -> CommitAck | StableError`；`append_wal(record) -> CommitAck`；`append_txn_journal(marker) -> CommitAck`（顺序保证）；`recover() -> RecoveryPlan`；`disk_pressure() -> PressureState`。

## 上游与下游依赖

- **上游**：[world-slot](../world-slot/README.md)（Snapshot/WAL 提交路径）、[maintenance-agent](../maintenance-agent/README.md)（维护落盘 ack 等待与 Migration 编排）、[process](../process/README.md)（恢复启动）。
- **下游**：[host-runtime](../host-runtime/README.md)（Worker 线程监督、Checkpoint 提醒 Timer）、[observability](../observability/README.md)（事件、Metrics、Failure Bundle 素材）。
...
- 拥有 IO/Persistence Worker 线程（数量为部署配置，经 host-runtime 受监督创建）。
- 持久队列（WAL/CommandLog/TxnJournal）独立于 Diagnostic 与 Audit 队列（容量属 SRV-D-014）；满载时**不丢**——拒绝新命令或触发进维护（架构源 ADR-011）。
...
- **SRV-D-009**（Checkpoint 周期与保留数量）：临时默认值为每 300 单调秒或每 6000 Tick 取先到者（触发恒在 Tick Barrier）、保留最近 3 个有效 Checkpoint；随 D-005 测量一并确认。
- **SRV-D-014**（durable 队列容量与背压阈值，WAL/Txn/Cmd 侧）：见 [modules/README.md](../README.md) §11.2。
