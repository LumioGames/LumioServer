...
ADR-056 / ADR-058：Rust 宿主是接力交付面，只托管与传输。世界是 Runtime `WorldManager`（`ServerBootstrap.Boot(instanceId)`）；绑定/查询/快照/Persist 只在 Runtime；定时只在 NativeCore 内核（wallClock + tickFrame）。

## 设计

- **会话表**：只保存 `connection_id → (netEntityId, connectionGeneration)` 与 egress。无账号键 `HashMap`/`BTreeMap`。顶号 = Runtime `account_already_online` → 旧连接 `ConnectionSuperseded` → `Rebind`；任一步失败不改会话表。
- **Runtime 消费**：CoreCLR `entity-chat-host` 装载 `Lumio.GameRuntime.Samples.Username.Server`，`ServerBootstrap.Boot(instanceId)` 持 `WorldManager`。转发 `Admit` / `Disconnect` / `Rebind` / `Expire` / `QueryAttribute` / `Tick` / `CaptureSnapshot` / `CreateFromSnapshot`。恢复前 `EcsRegistry.Current = GeneratedRegistry.Instance`。不反射 `_world`，不 overlay `ListBindings`。
- **定时**：owner loop 每 `OWNER_PUMP_INTERVAL_MS` `pump_wall_clock`（真单调钟）。生产 `SystemMonotonicClock` 无 `advance_ms`；测试注入 Fake `TestMonotonicClock`。Tick 号来自 Runtime `appliedTick`。五分钟断线保留走 `wallClock` one-shot。
...
- **解析 / 查询**：`ResolveByNetEntityId` 接受 Runtime 32-hex 与 C-1 u64。HostEntry 把 Runtime `OkEntity` 的 `netEntityId` 字段原样带回，不经 `ListBindings` 兜底。S5 unauthorized 走声明过的 claim-scoped `EntityIdentity.claimedMark`（`restrictedFlag` 未声明 → `RequestError`，不得冒充 Unauthorized）。
- **广播**：每连接有界 egress（64）；`try_send` 满则断开并记 `reason=backpressure`。Runtime 字节按严格 UTF-8 转发，禁止 `from_utf8_lossy`。Runtime FullSnapshot 空/失败 → admit 不 accepted。
- **日志**：JSON lines `{log-dir}/server.ndjson`，字段 `ts, tick, kind, netEntityId, messageId, roomSequence, appliedTick, reason`。kind：`host` / `admit` / `tick` / `event` / `expire` / `snapshot` / `superseded` / `rebind` / `restore`。`accountId` 省略或 hash，口令不得进日志。
- **instanceId**：宿主给 World Manager，默认 `0x1000000000000001`，来自 `--instance-id` / `LumioInstanceId` / `LUMIO_INSTANCE_ID`，写入 boot 响应与日志。发现路径经 `LumioRuntimeRoot` / `LUMIO_RUNTIME_ROOT`，禁止硬编码 `C:\Work`。
...
- **Persist**：`WorldManager.CaptureSnapshot` / `ServerBootstrap.Restore`（`CreateFromSnapshot`）。S7 进程 B 对 101 个 `ChatComponent.lastMessageText` 逐实体比对，结果写进程 B 日志 `kind=restore` / `restore.entity`。
